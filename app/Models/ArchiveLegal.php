...
class ArchiveLegal extends Model
{
    use HasFactory;
    ...
    public static function new($submit, $archive_id) {
        $archive = ArchiveLegal::where('date', Carbon::parse($submit->start_deadline)->format('Y-m-d'))->where('type', 1)->where('city_id', $submit->address->city_id)->first();
        if ($archive) {
            $archive->update([
                'submit_count' => $archive->submit_count + 1,
            ]);
        } else {
            $archive = new ArchiveLegal;
    ...
    public static function submitDelete($submit) {
        $archive = ArchiveLegal::where('date', Carbon::parse($submit->start_deadline)->format('Y-m-d'))->where('type', 1)->where('city_id', $submit->city_id)->first();
        if($archive) {
            $archive->update([
    ...
    public static function add($pakban) {
        $archive = ArchiveLegal::where('date', Carbon::parse($pakban->collected_at)->format('Y-m-d'))->where('type', 1)->where('city_id', $pakban->city_id)->first();
