...
class JobController extends Controller
{
    ...
    public function financialArchive(Request $request)
    {
        try {
            ...
            $rewards_first_bazist = WalletDetails::whereDate('created_at', $date)->where('type','first_submit_user')->sum('amount');
            ...
            $rewards_ref_bazist = WalletDetails::whereDate('created_at', $date)->where('details','LIKE','%پاداش معرف%')->whereIn('type',['submit_user_ref','deposit'])->sum('amount');
            ...
            $office_deposit = WalletDetails::whereDate('created_at', $date)->whereIn('user_id',[65332,64602])->where('method','واریز')->sum('amount');
            $office_withdraw = WalletDetails::whereDate('created_at', $date)->whereIn('user_id',[65332,64602])->where('method','برداشت')->sum('amount');
