...
namespace App\Livewire\Club\Auth;
...
class Login extends Component
{
    ...
    {
        return view('livewire.club.auth.login');
    }
    ...
    public function mobile()
    {
        ...
        if(!$user->getRoles()->intersect(['superadmin','marketer'])->count()){
            sendToast(0,'شما دسترسی ورود به پنل مدیریت را ندارید. (1002)');
        }
        elseif($user){
            $this->type = 'code';
        ...
        if (session()->exists('code')) {
            if ($this->code == session('code')) {
                ...
                if ($user) {
                    ...
                    return $this->redirect(route('cl.dashboard'));
                }
            }
            else{
            ...
            return $this->redirect(route('cl.login'));
        }
    }
}
