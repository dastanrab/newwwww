...
return new class extends Migration
{
    ...
    {
        Schema::create('receive_archives', function (Blueprint $table) {
            $table->bigIncrements('id');
            $table->unsignedBigInteger('city_id')->nullable()->default(1)->index('receive_archives_city_id_foreign');
            $table->dateTime('date');
            $table->integer('type');
            $table->integer('submit_count')->nullable();
            $table->integer('submit_done')->nullable();
            $table->integer('legal_submit_count')->default(0);
            $table->integer('illegal_submit_count')->default(0);
    ...
    {
        Schema::dropIfExists('receive_archives');
    }
};
