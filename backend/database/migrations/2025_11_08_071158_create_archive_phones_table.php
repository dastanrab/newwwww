...
return new class extends Migration
{
    ...
    {
        Schema::create('archive_phones', function (Blueprint $table) {
            $table->bigIncrements('id');
            $table->unsignedBigInteger('city_id')->default(1)->index('archive_phones_city_id_foreign');
            $table->unsignedBigInteger('receive_archive_id')->index('archive_phones_receive_archive_id_foreign');
            $table->dateTime('date');
            $table->integer('type');
    ...
    {
        Schema::dropIfExists('archive_phones');
    }
};
