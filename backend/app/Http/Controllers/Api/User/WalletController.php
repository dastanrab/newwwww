    ...
    public function asanPardakhtWithdraw(Request $request)
    {
        ...
        try {
            $data = DB::transaction(function () use($user,$wallet,$amountRial,$amountToman){
                ...
                $save = $wallet->save();
                if($save) {
                    $rawRecord = AsanPardakht::rawRecord($user->id, $amountRial, $city_id);
                    WalletDetails::create($city_id, $user->id, $wallet->id, 'cashout_to_aap', $rawRecord->id, $amountRial, $wallet->wallet * 10, 'برداشت', 'انتقال به کیف پول آپ');
                    ...
                    if($withdrawHresp->st == 0){
                        ...
                        if($withdrawVerifyHresp->st == 0){
                            $withdrawRecord = AsanPardakht::newWithdrawRecord($rawRecord->id,$user->id, $withdrawHresp, $city_id);
                        }
                        else{
...
//                    $amountToman = $amountToman - $taxToman;
