...
class ArchiveNotLegal extends Model
{
    use HasFactory;
    ...
    public static function new($submit, $archive_id) {
        $archive = ArchiveNotLegal::where('date', Carbon::parse($submit->start_deadline)->format('Y-m-d'))->where('type', 1)->where('city_id', $submit->address->city_id)->first();
        if ($archive) {
            $archive->update([
                'submit_count' => $archive->submit_count + 1,
            ]);
        } else {
            $archive = new ArchiveNotLegal;
    ...
    public static function submitDelete($submit) {
        $archive = ArchiveNotLegal::where('date', Carbon::parse($submit->start_deadline)->format('Y-m-d'))->where('type', 1)->where('city_id', $submit->city_id)->first();
        if($archive) {
            $archive->update([
    ...
    public static function add($pakban) {
        $archive = ArchiveNotLegal::where('date', Carbon::parse($pakban->collected_at)->format('Y-m-d'))->where('type', 1)->where('city_id', $pakban->city_id)->first();
